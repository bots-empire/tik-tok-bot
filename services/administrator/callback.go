...
func (h *AdminCallbackHandlers) Init(adminSrv *Admin) {
	...
	h.OnCommand("/change_text_tik_tok", adminSrv.ChangeTextTikTokCommand)
	h.OnCommand("/change_video_tik_tok", adminSrv.ChangeVideoTikTokCommand)
	h.OnCommand("/change_referral_amount_tik_tok", adminSrv.ChangeReferralAmountTikTokCommand)
	h.OnCommand("/change_bot_referral_amount", adminSrv.ChangeBotReferralAmount)
	h.OnCommand("/change_self_referral_amount", adminSrv.ChangeSelfReferralAmount)
...
func (a *Admin) ChangeTextTikTokCommand(s *model.Situation) error {
	lang := model.AdminLang(s.User.ID)

	db.RdbSetUser(s.BotLang, s.User.ID, "admin/set_text_tik_tok")
	err := a.msgs.SendAdminAnswerCallback(s.CallbackQuery, "send_the_video")
	if err != nil {
		return err
	}

	text := model.AdminSettings.GlobalParameters[s.BotLang].Parameters.TikTokText
	...
	text = a.bot.AdminText(lang, "set_new_tik_tok_text")

	return a.msgs.NewParseMessage(s.User.ID, text)
}

func (a *Admin) ChangeVideoTikTokCommand(s *model.Situation) error {
	lang := model.AdminLang(s.User.ID)
	key := "set_new_tik_tok_video"

	db.RdbSetUser(s.BotLang, s.User.ID, "admin/set_video_tik_tok")
	err := a.msgs.SendAdminAnswerCallback(s.CallbackQuery, "send_the_video")
	if err != nil {
		return err
	}
	...
	videoFileBytes := tgbotapi.FileID(model.AdminSettings.GlobalParameters[s.BotLang].Parameters.TikTokVideo)
...
func (a *Admin) ChangeReferralAmountTikTokCommand(s *model.Situation) error {
	lang := model.AdminLang(s.User.ID)
	text := a.bot.AdminText(lang, "change_referral_amount_tik_tok")

	markUp := msgs.NewIlMarkUp(
		msgs.NewIlRow(msgs.NewIlAdminButton("change_bot_referral_amount", "admin/change_bot_referral_amount")),
		msgs.NewIlRow(msgs.NewIlAdminButton("change_self_referral_amount", "admin/change_self_referral_amount")),
...
func (a *Admin) ChangeBotReferralAmount(s *model.Situation) error {
	lang := model.AdminLang(s.User.ID)

	parameter := a.bot.AdminText(lang, "change_bot_referral_amount")
	value := model.AdminSettings.GetParams(s.BotLang).ReferralFromBotVideoAmount

	text := a.adminFormatText(lang, "set_new_amount_text", parameter, value)

	db.RdbSetUser(s.BotLang, s.User.ID, "admin/change_bot_referral_amount")
	return a.msgs.NewParseMessage(s.User.ID, text)
}

func (a *Admin) ChangeSelfReferralAmount(s *model.Situation) error {
	lang := model.AdminLang(s.User.ID)

	parameter := a.bot.AdminText(lang, "change_self_referral_amount")
	value := model.AdminSettings.GetParams(s.BotLang).ReferralFromSelfVideoAmount

	text := a.adminFormatText(lang, "set_new_amount_text", parameter, value)

	db.RdbSetUser(s.BotLang, s.User.ID, "admin/change_self_referral_amount")
	return a.msgs.NewParseMessage(s.User.ID, text)
}
